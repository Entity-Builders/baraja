  ...
  return {
    ...
    configureServer(server: any) {
      server.middlewares.use(async (req: any, res: any, next: any) => {
        ...
        // ── Save deck settings (writes to JSON for HMR + DECKS static) ──
        if (req.url === '/api/admin/save-deck-settings' && req.method === 'POST') {
          try {
            ...
            if (updates.print_specs_overrides !== undefined) {
              deck.print_specs_overrides = updates.print_specs_overrides;
            }
            if (updates.design_template_overrides !== undefined) {
              deck.design_template_overrides = updates.design_template_overrides;
            }
